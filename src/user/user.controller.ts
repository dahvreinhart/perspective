...
export class UserController {
    ...
    @ApiOperation({
        ...
        description:
            'Fetch all the users in currently persisted in the database.',
    })
    ...
    async getUsers(
        @Query(new ValidationPipe(BasicValidationOptions))
        query: GetUsersSortQueryDTO,
    ): Promise<IUser[]> {
    ...
    @ApiOperation({
        ...
        description: 'Create a new user and persist it in the database.',
    })
    @ApiResponse({
        status: 201,
    ...
    async createUser(
        @Body(new ValidationPipe(BasicValidationOptions))
        UserCreationData: CreateUserBodyDTO,
    ): Promise<IUser> {
